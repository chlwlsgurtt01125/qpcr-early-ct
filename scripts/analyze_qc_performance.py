#!/usr/bin/env python3
"""
QC 결과와 예측 결과를 well_uid로 결합하여 버킷별 성능 분석 리포트 생성

outputs/qc_performance_analysis/ 폴더에 저장:
- qc_pred_merged.parquet (QC + Predictions 병합)
- bucket_performance.parquet (버킷별 성능 테이블)
...
def pick_predictions_file(project_root: Path, cutoff: int, qc_well_uids: set[str]) -> Path | None:
    """
    Choose predictions_long.parquet that best matches QC well_uid set.
    Priority: reports/ > app/data/reports/ > data/reports/
    Score: intersection count after cutoff filtering.
    """
    search_bases = [
        project_root / "reports",
        project_root / "app" / "data" / "reports",
    ...
    for base in search_bases:
        if base.exists():
            candidates.extend(list(base.glob("**/predictions_long.parquet")))
    ...
    for p in candidates:
        try:
            # 최소 컬럼만 읽기 (속도)
            df = pd.read_parquet(p, columns=["well_id", "cutoff"])
            ...
            if df.empty:
                ...
                well_ids = set(df["well_id"].astype(str).tolist())
                hit = len(well_ids.intersection(qc_well_uids))
                n = len(well_ids)
        except Exception:
            continue

        # 교집합이 많은 파일 우선. 동률이면 표본 수 큰 파일 우선.
        if (hit > best_hit) or (hit == best_hit and n > best_n):
            best_hit, best_n = hit, n
            best_path = p

    # 교집합이 0이면 사실상 QC와 조인 불가 파일 -> None 처리
    if best_hit <= 0:
        return None

    print(f"\n📂 Auto-selected predictions (by QC overlap): {best_path}")
    print(f"   overlap wells: {best_hit} / QC={len(qc_well_uids)} / pred_unique={best_n}")
    return best_path
...
def create_visualization(bucket_perf: pd.DataFrame, output_dir: Path):
    ...
    if len(qc_status_perf) > 0:
        ...
        print(f"   ✅ {fig_dir / 'performance_by_qc_status.png'}")
        plt.close()
    
    # === Figure 2: Fail Reason별 성능 비교 (Top 10) ===
    ...
    if len(fail_reason_perf) > 0:
        ...
        print(f"   ✅ {fig_dir / 'performance_by_fail_reason.png'}")
        plt.close()
    
    # === Figure 3: Ct Bin별 성능 비교 ===
    ...
    if len(ct_bin_perf) > 0:
        ...
        print(f"   ✅ {fig_dir / 'performance_by_ct_bin.png'}")
        plt.close()


def main():
    parser = argparse.ArgumentParser(description='QC 결과와 예측 결과를 결합하여 성능 분석')
    parser.add_argument('--predictions', type=str, 
                       default=None,
                       help='Predictions parquet 파일 경로 (기본: 자동 탐색)')
    parser.add_argument('--cutoff', type=int, default=20,
                       help='분석할 cutoff (기본: 20)')
    args = parser.parse_args()
    
    print("=" * 60)
    print("QC + Predictions 버킷별 성능 분석 (well_uid 기반)")
    print("=" * 60)
    ...
    if not qc_catalog_path.exists():
        print("\n❌ QC catalog not found. Run: python3 scripts/save_qc_results.py")
        return
    ...
    # 2. Predictions 경로 결정 (QC overlap 기반 자동 선택)
    if args.predictions is None:
        qc_well_uids = set(qc_catalog["well_uid"].astype(str).unique().tolist())
        predictions_path = pick_predictions_file(PROJECT_ROOT, args.cutoff, qc_well_uids)
        if predictions_path is None:
            print("\n❌ No compatible predictions_long.parquet found (QC overlap = 0).")
            print("   Try: --predictions reports/model_server_latest_xgb/predictions_long.parquet")
            return
    else:
    ...
    # 1. QC Catalog 로드
    if not qc_catalog_path.exists():
        print(f"\n❌ QC catalog not found. Run: python scripts/save_qc_results.py")
        return
    ...
    # predictions의 well_id를 well_uid로 사용
    if 'well_id' in predictions.columns:
        ...
        print(f"   ❌ No well_id column in predictions!")
        return
    
    # 3. well_uid로 merge
    print("\n🔄 Merging QC catalog with predictions on well_uid...")
    ...
    if pred_ct_col is None:
        print(f"   ❌ No predicted Ct column found. Columns: {merged.columns.tolist()}")
        return
    ...
    # 운영용 최종 출력만 따로 저장 (보고/업로드용)
    ops_cols = [
        "well_uid", "run_id", "Well", "channel",
        true_ct_col, pred_ct_col,
        "qc_status", "fail_reason", "ct_bin",
        "final_decision", "ct_output", "decision_note",
    ]
    ops_cols = [c for c in ops_cols if c in merged.columns]  # 안전
    
    ops = merged[ops_cols].copy()
    ops = ops.rename(columns={true_ct_col: "true_ct", pred_ct_col: "pred_ct"})
    
    ops_path = output_dir / f"ops_decisions_cutoff_{args.cutoff}.csv"
    ...
    print(f"   P(|error| ≤ 0.5): {(merged['abs_error'] <= 0.5).sum() / len(merged) * 100:.1f}%")
    print(f"   P(|error| ≤ 1.0): {(merged['abs_error'] <= 1.0).sum() / len(merged) * 100:.1f}%")
    ...
    # -------------------------
    # 운영용(ops) 파일 저장 (Streamlit이 바로 읽는 파일)
    # -------------------------
    ops_cols = [
        "well_uid", "run_id", "Well", "channel",
        "qc_status", "fail_reason", "ct_bin",
    ...
    ops_cols = [c for c in ops_cols if c in merged.columns]
    ops_df = merged[ops_cols].copy()
    
    ops_parquet = output_dir / f"ops_decisions_cutoff_{args.cutoff}.parquet"
    ops_csv = output_dir / f"ops_decisions_cutoff_{args.cutoff}.csv"
    
    ops_df.to_parquet(ops_parquet, index=False)
    ops_df.to_csv(ops_csv, index=False)
    
    print("\n💾 Saving ops decisions (for Streamlit)...")
    print(f"   ✅ {ops_parquet}")
    print(f"   ✅ {ops_csv}")
    
    # 6. 버킷별 성능 계산
    print("\n🔄 Calculating bucket metrics...")
    bucket_perf = calculate_bucket_metrics(merged)
    print(f"   Generated {len(bucket_perf)} bucket metrics")
    
    # 7. 저장
    print("\n💾 Saving bucket performance...")
    
    bucket_perf.to_parquet(output_dir / "bucket_performance.parquet", index=False)
    print(f"   ✅ {output_dir / 'bucket_performance.parquet'}")
    ...
    summary_lines.append(f"P(|error| ≤ 0.5): {(merged['abs_error'] <= 0.5).sum() / len(merged) * 100:.1f}%")
    summary_lines.append(f"P(|error| ≤ 1.0): {(merged['abs_error'] <= 1.0).sum() / len(merged) * 100:.1f}%")
    ...
    for status in ['PASS', 'FAIL', 'FLAG']:
        ...
        if len(status_df) > 0:
            ...
            summary_lines.append(f"  P(≤0.5): {(status_df['abs_error'] <= 0.5).sum() / len(status_df) * 100:.1f}%")
            summary_lines.append(f"  P(≤1.0): {(status_df['abs_error'] <= 1.0).sum() / len(status_df) * 100:.1f}%")
